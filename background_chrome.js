...
const API_BASE_DEFAULT   = ''; // Cấu hình API URL trong tab Tài khoản proxy
const IP_CHECK_HOST      = 'api.ipify.org';
const KS_RULE_BASE       = 100;
const KS_RULE_MAX_SITES  = 200;
const KS_AHEAD_MS        = 5 * 60 * 1000; // kích hoạt kill switch trước 5 phút
...
// Kill switch kích hoạt sớm trước KS_AHEAD_MS để tránh gap leak
function shouldBlock(proxyData) {
  if (!proxyData) return true;
  if (!proxyData.expiresAt) return false;
  const remaining = new Date(proxyData.expiresAt).getTime() - Date.now();
  return remaining <= KS_AHEAD_MS;
}
...
async function updateKillSwitchRules() {
  ...
  if (!shouldBlock(proxyData)) {
    // Proxy còn hơn 5 phút → xóa kill switch
    await chrome.declarativeNetRequest.updateDynamicRules({ removeRuleIds: removeIds, addRules: [] });
    return;
  }

  // Proxy hết hạn hoặc sắp hết hạn (< 5 phút) → block
  const remaining = proxyData?.expiresAt ? new Date(proxyData.expiresAt).getTime() - Date.now() : -1;
  const reason    = !proxyData ? 'no_proxy' : remaining > 0 ? 'expiring' : 'expired';
...
// MV3 + webRequestAuthProvider: dùng asyncBlocking + callback để Chrome chờ credentials
try {
  chrome.webRequest.onAuthRequired.addListener(
    (details, callback) => {
      if (!details.isProxy) { callback({}); return; }
      chrome.storage.local.get('proxyData').then(stored => {
        const p = stored.proxyData;
        callback(p && p.username
...
} catch(e) { /* auth listener not supported */ }
...
async function syncOnStartup() {
  ...
  scheduleExpiryAlarm(stored.proxyData);
}

chrome.runtime.onStartup.addListener(syncOnStartup);
chrome.runtime.onInstalled.addListener(syncOnStartup);

// ─── Alarm: kích hoạt kill switch đúng lúc proxy hết hạn ────────────────────
// Không poll liên tục — set alarm chính xác vào thời điểm hết hạn.
// Fallback alarm mỗi 5 phút để sync lại sau khi service worker restart.

function scheduleExpiryAlarm(proxyData) {
  chrome.alarms.clear('proxyExpired');
  if (!proxyData?.expiresAt) return;
  const expiryMs = new Date(proxyData.expiresAt).getTime();
  const alarmAt  = expiryMs - KS_AHEAD_MS; // 5 phút TRƯỚC khi hết hạn
  if (alarmAt > Date.now()) {
    chrome.alarms.create('proxyExpired', { when: alarmAt });
  } else {
    // Còn dưới 5 phút hoặc đã hết hạn → kích hoạt kill switch ngay
    updatePacScript();
    updateKillSwitchRules();
  }
}

// Fallback: sync mỗi 5 phút
chrome.alarms.create('proxySync', { periodInMinutes: 5 });

chrome.alarms.onAlarm.addListener(async (alarm) => {
  if (alarm.name === 'proxyExpired' || alarm.name === 'proxySync') {
    await updatePacScript();
    await updateKillSwitchRules();
  }
});
...
async function apiGet(params) {
  ...
  const apiUrl = stored.apiUrl || API_BASE_DEFAULT;
  if (!apiUrl) throw new Error('Chưa cấu hình API URL trong Cài đặt');
...
async function handleMessage(msg) {
  switch (msg.type) {
    ...
    case 'getState': {
      ...
      return { proxyData: p, groups: s.groups || [], remainingMs: remaining, proxyIp: s.proxyIp || null };
    }
